...
#pragma region ambient speech data organised old
		const std::vector<AmbientSpeechDataS> vSpeechData{{"Frightened", "MAUDE", "GENERIC_FRIGHTENED_HIGH", "SPEECH_PARAMS_FORCE"},
...
#pragma endregion
		...
		bool PopulateVoiceData()
		{
			...
			std::sort(vVoiceData.begin(), vVoiceData.end(), [](const AmbientVoice_t& a, const AmbientVoice_t& b) -> bool {
		...
		void Sub_AmbientSpeechPlayer_InVoice()
		{
			...
			{
				if (_currSpeechParamIndex < vSpeechParams.size() - 1)
					_currSpeechParamIndex++;
			}
			if (bSpeechParam_minus)
			{
				if (_currSpeechParamIndex > 0)
					_currSpeechParamIndex--;
			}
