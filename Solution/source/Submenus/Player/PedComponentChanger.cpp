...
namespace sub
{
	...
	{
		bool pressed = false;
		AddOption(text, pressed, nullFunc, SUB::COMPONENTS2, true, true);
		if (pressed)
		{
	...
	{
		bool pressed = false;
		AddOption(text, pressed, nullFunc, SUB::COMPONENTSPROPS2);
		if (pressed)
		{
			_hud_color_index = index;
		}
	...
	void ComponentChanger_()
	{
		...
		bool ComponentChanger_random = 0, comp_front_view = 0, ComponentChanger_default = 0, ComponentChanger_online_police_m = 0, ComponentChanger_online_robber_m = 0, ComponentChanger_online_garbage_m = 0, ComponentChanger_online_police_f = 0, ComponentChanger_offline_police_michael = 0, ComponentChanger_offline_firefighter_michael = 0;
		...
		case PedHash::Michael: AddBreak("---Premade Outfits---"); break;
		}
		...
		if (comp_front_view)
		{
			...
			else
			{
				...
				gmCam.Position_set(World::RenderingCamera_get().Handle() == 0 ? GameplayCamera::Position_get() :
				                                                                World::RenderingCamera_get().Position_get());
				gmCam.Rotation_set(World::RenderingCamera_get().Handle() == 0 ? GameplayCamera::Rotation_get() :
				                                                                World::RenderingCamera_get().Rotation_get());
	...
	{
		bool compon_plus = 0, compon_minus = 0;

		int compon_drawable_current = GET_PED_DRAWABLE_VARIATION(local_ped_id, _hud_color_index), compon_texture_current = GET_PED_TEXTURE_VARIATION(local_ped_id, _hud_color_index), compon_palette_current = GET_PED_PALETTE_VARIATION(local_ped_id, _hud_color_index);

		int compon_drawable_old = compon_drawable_current, compon_texture_old = compon_texture_current, compon_palette_old = compon_palette_current;
		...
		case 2:
			...
			else if (compon_minus)
			{
				if (compon_texture_current > 0)
				{
					...
					compon_texture_current = GET_NUMBER_OF_PED_TEXTURE_VARIATIONS(local_ped_id, _hud_color_index, compon_drawable_current) - 1;
			}
			break;
		case 3:
			if (compon_plus)
			{
				...
				if (compon_palette_current > 0)
				{
					...
					compon_palette_current = 10;
			break;
		}

		if (compon_drawable_old != compon_drawable_current || compon_texture_old != compon_texture_current || compon_texture_old != compon_palette_current)
		{
	...
	void ComponentChangerProps2_()
	{
		...
		int prop_type_current = GET_PED_PROP_INDEX(local_ped_id, _hud_color_index, 0), prop_texture_current = GET_PED_PROP_TEXTURE_INDEX(local_ped_id, _hud_color_index);
		int prop_type_old = prop_type_current, prop_texture_old = prop_texture_current;
		...
		case 2:
			...
			else if (compon_minus)
			{
				if (prop_texture_current > 0)
				{
					...
					prop_texture_current = GET_NUMBER_OF_PED_PROP_TEXTURE_VARIATIONS(local_ped_id, _hud_color_index, prop_type_current) - 1;
			}
			break;
		}
...
#pragma endregion
		...
		void Sub_BoneSelection()
		{
			...
			{
				bPressed = false;
				AddTickol(bn.name, _boneToUse == bn.boneid, bPressed, bPressed, TICKOL::SKULL_DM, TICKOL::NONE, true);
				if (bPressed) // Gxt test
				{
...
#pragma region arrays
		...
		const std::vector<std::string> vCaptions_facialFeatures{{"Nose Width"}, {"Nose Bottom Height"}, {"Nose Tip Length"}, {"Nose Bridge Depth"}, {"Nose Tip Height"}, {"Nose Broken"}, {"Brow Height"}, {"Brow Depth"}, {"Cheekbone Height"}, {"Cheekbone Width"}, {"Cheek Depth"}, {"Eye Size"}, {"Lip Thickness"}, {"Jaw Width"}, {"Jaw Shape"}, {"Chin Height"}, {"Chin Depth"}, {"Chin Width"}, {"Chin Indent"}, {"Neck Width"}};
#pragma endregion
		...
		uint8_t GetPedHeadOverlayColourType(const PedHeadOverlay& overlayIndex)
		{
			...
			case PedHeadOverlay::Makeup: return 1; break;
			case PedHeadOverlay::Blush:
			case PedHeadOverlay::Lipstick: return 2; break;
			}
			return 0;
		}
		bool DoesPedModelSupportHeadFeatures(const GTAmodel::Model& pedModel)
		{
			...
			{
			case PedHash::FreemodeMale01:
			case PedHash::FreemodeFemale01: return true; break;
			}
			return false;
		}
		...
		void Sub_Main()
		{
			...
			if (headBlend.shapeFirstID < 0 || headBlend.shapeFirstID > max_shapeAndSkinIDs || headBlend.shapeSecondID < 0
			    || headBlend.shapeSecondID > max_shapeAndSkinIDs || headBlend.shapeThirdID < 0 || headBlend.shapeThirdID > max_shapeAndSkinIDs
			    || headBlend.skinFirstID < 0 || headBlend.skinFirstID > max_shapeAndSkinIDs || headBlend.skinSecondID < 0
			    || headBlend.skinSecondID > max_shapeAndSkinIDs || headBlend.skinThirdID < 0 || headBlend.skinThirdID > max_shapeAndSkinIDs)
			{
			...
			if (hairColour_plus)
			{
				if (_pedHead->hairColour < max_hairColours)
					_pedHead->hairColour++;
				SET_PED_HAIR_TINT(ped.Handle(), _pedHead->hairColour, _pedHead->hairColourStreaks);
			}
			if (hairColour_minus)
			{
				if (_pedHead->hairColour > 0)
					_pedHead->hairColour--;
				SET_PED_HAIR_TINT(ped.Handle(), _pedHead->hairColour, _pedHead->hairColourStreaks);
			}
			if (hairColourStreaks_plus)
			{
				if (_pedHead->hairColourStreaks < max_hairColours)
					_pedHead->hairColourStreaks++;
				SET_PED_HAIR_TINT(ped.Handle(), _pedHead->hairColour, _pedHead->hairColourStreaks);
			}
			if (hairColourStreaks_minus)
			{
				if (_pedHead->hairColourStreaks > 0)
					_pedHead->hairColourStreaks--;
				SET_PED_HAIR_TINT(ped.Handle(), _pedHead->hairColour, _pedHead->hairColourStreaks);
			}
		...
		void Sub_HeadOverlays_InItem()
		{
			...
			if (overlay_plus)
			{
				if (currentOverlayValue < max_overlays)
				{
					...
					SET_PED_HEAD_OVERLAY_TINT(ped.Handle(),
					    overlayIndex,
					    colourType,
		...
		void Sub_SkinTone() // HEAD_BLEND
		{
			...
			if (shapeFirstID_plus)
			{
				if (blendData.shapeFirstID < max_ids)
				{
					blendData.shapeFirstID++;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			}
			if (shapeFirstID_minus)
			{
				if (blendData.shapeFirstID > 0)
				{
					blendData.shapeFirstID--;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			...
			if (shapeSecondID_plus)
			{
				if (blendData.shapeSecondID < max_ids)
				{
					blendData.shapeSecondID++;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			}
			if (shapeSecondID_minus)
			{
				if (blendData.shapeSecondID > 0)
				{
					blendData.shapeSecondID--;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			...
			if (skinFirstID_plus)
			{
				if (blendData.skinFirstID < max_ids)
				{
					blendData.skinFirstID++;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			}
			if (skinFirstID_minus)
			{
				if (blendData.skinFirstID > 0)
				{
					blendData.skinFirstID--;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			...
			if (skinSecondID_plus)
			{
				if (blendData.skinSecondID < max_ids)
				{
					blendData.skinSecondID++;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			}
			if (skinSecondID_minus)
			{
				if (blendData.skinSecondID > 0)
				{
					blendData.skinSecondID--;
					UpdatePedHeadBlendData(ped, blendData, true);
				}
			...
			if (shapeMix_plus)
			{
				if (blendData.shapeMix < max_mix)
				{
					blendData.shapeMix += mix_amountToChange;
					UpdatePedHeadBlendData(ped, blendData, false);
				}
			}
			if (shapeMix_minus)
			{
				if (blendData.shapeMix > min_mix)
				{
					blendData.shapeMix -= mix_amountToChange;
					UpdatePedHeadBlendData(ped, blendData, false);
				}
			...
			if (skinMix_plus)
			{
				if (blendData.skinMix < max_mix)
				{
					blendData.skinMix += mix_amountToChange;
					UpdatePedHeadBlendData(ped, blendData, false);
				}
			}
			if (skinMix_minus)
			{
				if (blendData.skinMix > min_mix)
				{
					blendData.skinMix -= mix_amountToChange;
					UpdatePedHeadBlendData(ped, blendData, false);
				}
	...
	namespace ComponentChanger_Outfit_catind
	{
		...
		bool Create(GTAentity ped, std::string filePath)
		{
			...
			if (oldXml.load_file((const char*)filePath.c_str()).status == pugi::status_ok)
			{
				...
				bAddAttachmentsToSpoonerDb =
				    nodeOldRoot.child("SpoonerAttachments").attribute("SetAttachmentsPersistentAndAddToSpoonerDatabase").as_bool(bAddAttachmentsToSpoonerDb);
		...
		bool Apply(GTAped ep, const std::string& filePath, bool applyModelAndHead, bool applyProps, bool applyComps, bool applyDecals, bool applyDamageTextures, bool applyAttachedEntities)
		{
			...
			if (applyAttachedEntities)
			{
				...
				bool bAddAttachmentsToSpoonerDb =
				    nodeAttachments.attribute("SetAttachmentsPersistentAndAddToSpoonerDatabase").as_bool(false);
				...
				for (auto nodeAttachment = nodeAttachments.first_child(); nodeAttachment;
				     nodeAttachment      = nodeAttachment.next_sibling())
				{
					...
					sub::Spooner::EntityManagement::AttachEntity(e.e,
					    ep,
	...
	void ComponentChanger_Outfits()
	{
		...
		if (_dir.empty())
			_dir = GetPathffA(Pathff::Outfit, false);
	...
	void ComponentChanger_Outfits2()
	{
		...
		bool outfits2_apply = 0, outfits2_applyAllFeatures = 0, outfits2_applyModel = 0, outfits2_overwrite = 0, outfits2_rename = 0, outfits2_delete = 0;
