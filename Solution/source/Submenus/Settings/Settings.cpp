...
namespace sub
{
	...
	void SettingsColours2()
	{
		...
		case 1:
		case 2:
		case 3: Add_preset_colour_options_previews(settings_rgba->R, settings_rgba->G, settings_rgba->B); break;
		}
		...
		if (settings_hud_c_apply)
		{
			GET_HUD_COLOUR(settings_hud_c, &settings_rgba->R, &settings_rgba->G, &settings_rgba->B, &inull);
			return;
		}
	}
	...
	void SettingsLanguage()
	{
		...
		{
			bool bPressed = false;
			AddTickol(l.GetName(), Language::selectedLang == &l, bPressed, bPressed);
			if (bPressed)
			{
	...
	namespace SettingsThemes_catind
	{
		...
		public:
			...
			MenyooTheme(bool _grads, bool _rainbow, bool _thinLineOverFooter, RGBA _ttbox, RGBA _bgbox, RGBA _tttext, RGBA _optext, RGBA _seltext, RGBA _opbreak, RGBA _opcount, RGBA _selhi, RGBA _pedtrackers, int8_t _f_title, int8_t _f_options, int8_t _f_selection, int8_t _f_breaks, int8_t _f_xyzh, int8_t _f_speedo)
			{
				grads              = _grads;
				rainbow            = _rainbow;
			...
			bool IsActive()
			{
				return Menu::gradients == grads && loop_RainbowBoxes == rainbow && Menu::thin_line_over_screct == thinLineOverFooter &&

				    titlebox == ttbox && BG == bgbox && titletext == tttext && optiontext == optext && selectedtext == seltext && optionbreaks == opbreak && optioncount == opcount && selectionhi == selhi && _globalPedTrackers_Col == pedtrackers &&

				    font_title == f_title && font_options == f_options && font_selection == f_selection && font_breaks == f_breaks && font_xyzh == f_xyzh && font_speedo == f_speedo;
			}
			...
			bool operator==(MenyooTheme const value2)
			{
				...
				return value1.grads == value2.grads && value1.rainbow == value2.rainbow
				    && value1.thinLineOverFooter == value2.thinLineOverFooter &&

				    value1.ttbox == value2.ttbox && value1.bgbox == value2.bgbox && value1.tttext == value2.tttext
				    && value1.optext == value2.optext && value1.seltext == value2.seltext && value1.opbreak == value2.opbreak
				    && value1.opcount == value2.opcount && value1.selhi == value2.selhi && value1.pedtrackers == value2.pedtrackers &&

				    value1.f_title == value2.f_title && value1.f_options == value2.f_options
				    && value1.f_selection == value2.f_selection && value1.f_breaks == value2.f_breaks
				    && value1.f_xyzh == value2.f_xyzh && value1.f_speedo == value2.f_speedo;
			}
