...
namespace sub::TeleportLocations_catind
{
	...
	{
		const std::vector<std::string> vGarageIds{{"1"}, {"2"}, {"3"}, {"Autoshop"}};
		...
		void CreateOfficeGarage(OfficeGarageInfoStructure& garageInfo)
		{
			...
			if (garageInfo.location != nullptr)
			{
				...
				ACTIVATE_INTERIOR_ENTITY_SET(interior,
				    (vNumStyleOptions[garageInfo.numStyleOption].value + "_N" + std::to_string(garageInfo.garageId + 1)).c_str());
		...
		void Sub_OfficeGarages()
		{
			...
			for (auto& loc : vLocations)
			{
				...
				AddOption(loc.first, bGrpLocPressed, nullFunc, SUB::TELEPORTOPS_OFFICEGARAGES_INLOC);
				if (bGrpLocPressed)
				{
		...
		void Sub_OfficeGarages_InLoc()
		{
			...
			if (bGarageId_plus)
			{
				if (currentGarageInfo.garageId < vGarageIds.size() - 1)
				{
					currentGarageInfo.garageId++;
				}
			}
			if (bGarageId_minus)
			{
				if (currentGarageInfo.garageId > 0)
				{
					currentGarageInfo.garageId--;
				}
			}
			...
			if (currentGarageInfo.garageId == 3) //Autoshop
			{
				...
				if (bFloorOption_plus)
				{
					if (currentGarageInfo.floorOption < vFloorOptions.size() - 1)
					{
						currentGarageInfo.floorOption++;
					}
				}
				if (bFloorOption_minus)
				{
					if (currentGarageInfo.floorOption > 0)
					{
						currentGarageInfo.floorOption--;
					}
				}
			...
			else
			{
				...
				if (bDecorOption_plus)
				{
					if (currentGarageInfo.decorOption < vDecorOptions.size() - 1)
					{
						currentGarageInfo.decorOption++;
					}
				}
				if (bDecorOption_minus)
				{
					if (currentGarageInfo.decorOption > 0)
					{
						currentGarageInfo.decorOption--;
					}
				}
				...
				if (bLightingOption_plus)
				{
					if (currentGarageInfo.lightingOption < vLightingOptions.size() - 1)
					{
						currentGarageInfo.lightingOption++;
					}
				}
				if (bLightingOption_minus)
				{
					if (currentGarageInfo.lightingOption > 0)
					{
						currentGarageInfo.lightingOption--;
					}
				}
				...
				AddTexter("Numbering Style",
				    0,
				    std::vector<std::string>{vNumStyleOptions[currentGarageInfo.numStyleOption].name},
				    null,
				    bNumStyle_plus,
				    bNumStyle_minus);
				if (bNumStyle_plus)
				{
					if (currentGarageInfo.numStyleOption < vNumStyleOptions.size() - 1)
					{
						currentGarageInfo.numStyleOption++;
					}
				}
				if (bNumStyle_minus)
				{
					if (currentGarageInfo.numStyleOption > 0)
					{
						currentGarageInfo.numStyleOption--;
					}
				}
