...
#pragma endregion
		...
		void AddvcatOption_(const std::string& text, uint8_t index, bool* extra_option_code)
		{
			...
			bool pressed = false;
			AddOption(text, pressed, nullFunc, -1, true);
			if (pressed)
			{
		...
		void AddVSpawnOption_(const std::string& text, const GTAmodel::Model& vehModel, Ped ped)
		{
			...
			if (pressed)
			{
				...
				set_vehicle_max_upgrades(vehicle,
				    _globalSpawnVehicle_autoUpgrade,
				    _globalSpawnVehicle_invincible,
				    _globalSpawnVehicle_plateType,
				    _globalSpawnVehicle_plateTexter_value == 0 ? _globalSpawnVehicle_plateText : "",
				    _globalSpawnVehicle_neonToggle,
				    _globalSpawnVehicle_neonCol.R,
				    _globalSpawnVehicle_neonCol.G,
				    _globalSpawnVehicle_neonCol.B,
				    _globalSpawnVehicle_PrimCol,
				    _globalSpawnVehicle_SecCol);
	...
	void SpawnVehicle_()
	{
		...
		if (spnrandom || spawnvehicle_input)
		{
			...
			if (model.IsInCdImage() && model.IsVehicle())
			{
				...
				set_vehicle_max_upgrades(spawnedVehicle,
				    _globalSpawnVehicle_autoUpgrade,
				    _globalSpawnVehicle_invincible,
				    _globalSpawnVehicle_plateType,
				    _globalSpawnVehicle_plateTexter_value == 0 ? _globalSpawnVehicle_plateText : "",
	...
	{
		bool veh_plate_plus = 0, veh_plate_minus = 0, veh_plate_text_set = 0, veh_plate_text_plus = 0, veh_plate_text_minus = 0, set_mspaint_index_10 = 0, set_mspaint_index_11 = 0, set_rgbcarcol_index_9 = 0;
		...
		AddOption("RGB Colour", set_rgbcarcol_index_9, nullFunc, SUB::MSPAINTS_RGB);
		if (*Menu::currentopATM == Menu::printingop)
			Add_preset_colour_options_previews(_globalSpawnVehicle_neonCol);
		...
		AddOption("Reload Previews", bReloadPreviewsPressed);
		if (bReloadPreviewsPressed)
		{
			SpawnVehicle_catind::PopulateVehicleBmps();
			FolderPreviewBmps_catind::PopulateFolderBmps();
		}

		if (set_mspaint_index_10)
			ms_curr_paint_index = 10;
		if (set_mspaint_index_11)
			ms_curr_paint_index = 11;
		if (set_rgbcarcol_index_9)
			bit_MSPaints_RGB_mode = 9;

		if (veh_plate_plus)
		{
			if (_globalSpawnVehicle_plateType < 5)
				_globalSpawnVehicle_plateType++;
			return;
		}
		if (veh_plate_minus)
		{
			if (_globalSpawnVehicle_plateType > 0)
				_globalSpawnVehicle_plateType--;
			return;
		}

		if (veh_plate_text_plus)
		{
			if (plateTexter_value < 1)
				plateTexter_value++;
			return;
		}
		if (veh_plate_text_minus)
		{
	...
	void SpawnVehicle_AllCatsSub()
	{
		...
		for (auto& vehModel : *tempvecp)
		{
			...
			case SUB::SPOONER_SPAWN_VEHICLE:
				...
				break;
			case SUB::SPAWNVEHICLE: AddVSpawnOption_(vehModel.VehicleDisplayName(true), vehModel, local_ped_id); break;
			}
			/// one submenu back >>>
			switch (Menu::currentsub_ar[Menu::currentsub_ar_index - 1]) /// -1
			{
			case SUB::OBJECTGUN: AddogunOption_(vehModel.VehicleDisplayName(true), vehModel, nullptr, false); break;
			case SUB::KABOOMGUN:
				AddkgunOption_(vehModel.VehicleDisplayName(true), vehModel.hash, nullptr, false);
				break;
			case SUB::MSENGINESOUND:
				AddTickol(vehModel.VehicleDisplayName(true), get_vehicle_engine_sound_name(_hud_color_index) == vehModel.VehicleDisplayName(false), bSetEngSoundPressed, bSetEngSoundPressed);
				if (bSetEngSoundPressed)
				{
			...
			if (Menu::printingop == *Menu::currentopATM)
			{
				...
				if (Menu::bit_controller)
				{
					...
					{
						!bIsAFav ? SpawnVehicle_AddVehicleModelToFavourites(vehModel,
						    Game::InputBox("", 28U, "Enter custom name:", vehModel.VehicleDisplayName(true))) :
						           SpawnVehicle_RemoveVehicleModelFromFavourites(vehModel);
					}
				}
				else
				{
	...
	void SpawnVehicle_Favourites()
	{
		...
		AddOption("Add New Vehicle Model", bInputAdd);
		if (bInputAdd)
		{
		...
		if (nodeRoot.first_child())
		{
			...
			for (auto nodeLocToLoad = nodeRoot.first_child(); nodeLocToLoad; nodeLocToLoad = nodeLocToLoad.next_sibling())
			{
				...
				if (!_searchStr.empty())
				{
					if (boost::to_upper_copy(vehModelName).find(_searchStr) == std::string::npos)
						continue;
				}
				...
				else
				{
					...
					case SUB::SPOONER_SPAWN_VEHICLE:
						...
						break;
					case SUB::SPAWNVEHICLE: AddVSpawnOption_(vehDisplayName, vehModel, local_ped_id); break;
					}
					/// one submenu back >>>
					switch (Menu::currentsub_ar[Menu::currentsub_ar_index - 1]) /// -1
					{
					case SUB::OBJECTGUN: AddogunOption_(vehDisplayName, vehModel, &null, false); break;
					case SUB::KABOOMGUN: AddkgunOption_(vehDisplayName, vehModel.hash, &null, false); break;
					case SUB::MSENGINESOUND:
						AddTickol(vehDisplayName, get_vehicle_engine_sound_name(_hud_color_index) == vehModel.VehicleDisplayName(false), bSetEngSoundPressed, bSetEngSoundPressed);
						if (bSetEngSoundPressed)
						{
							GTAvehicle veh12 = _hud_color_index;
							veh12.RequestControl(200);
	...
	namespace VehicleSaver_catind
	{
					...
					if (e.AttachmentArgs.isAttached)
					{
						...
						if (sub::Spooner::EntityManagement::GetEntityThisEntityIsAttachedTo(e.Handle, att))
						{
							if (att.Handle() == vSpoonerAttachmentsSaved[bei].Handle()) // baseEntToCheck is vSpoonerAttachmentsSaved[bei]
							{
								...
								std::string attachedToHandleStr =
								    nodeAttachment.child("Attachment").child("AttachedTo").text().as_string();
		...
		void VehSaver_ReadFromFile(std::string filePath, GTAentity ped)
		{
			...
			if (nodeVehicle.child("IsDriverVisible"))
			{
				...
				case 1:
					...
					break;                            // Retain
				case 2: ped.SetVisible(false); break; // ForceOff
				case 3: ped.SetVisible(true); break;  // ForceOn
				}
			}
			if (nodeVehicle.child("MaxHealth"))
				ev.MaxHealth_set(nodeVehicle.child("MaxHealth").text().as_int());
			if (nodeVehicle.child("Health"))
				ev.Health_set(nodeVehicle.child("Health").text().as_int());
			...
			else
			{
				...
				for (auto& e : vSpawnedAttachments)
				{
					if (!_globalSpawnVehicle_persistent)
		...
		int saveCarVars(GTAvehicle vehicle)
		{
			...
			int color1 = vehicle.PrimaryColour_get(), color2 = vehicle.SecondaryColour_get(), color3 = vehicle.PearlescentColour_get(),
			    color4 = vehicle.RimColour_get(), color5 = vehicle.InteriorColour_get(), color6 = vehicle.DashboardColour_get();
		...
		void Sub_VehSaver()
		{
			...
			AddTexter("Driver Visibility",
			    _driverVisibilityTexterIndex,
			    std::vector<std::string>{"FileDecides",
			        "Retain"
			        "ForceOff",
			        "ForceOn"},
			    null,
