...
#pragma endregion
			...
			if (pressed)
			{
				...
				if (HAS_NAMED_PTFX_ASSET_LOADED(asset))
				{
					...
					else
					{
						...
						START_NETWORKED_PARTICLE_FX_NON_LOOPED_AT_COORD((PCHAR)effect.fx.c_str(),
						    coord.x,
						    coord.y,
						    coord.z,
						    0.0f,
		...
		void __AddOption_loopOnEntity(const PtfxS& effect, GTAentity entity)
		{
			...
			bool pressed = false;
			AddTickol(effect.name, tickolTrue, pressed, pressed);
			if (pressed)
			{
		...
		void PTFXSub()
		{
			...
			{
			case SUB::PLAYEROPS:
			case SUB::SPOONER_SELECTEDENTITYOPS: AddOption("Loop On Entity", null, nullFunc, SUB::PTFXSUB); break;
			case SUB::PTFXSUB:
			{
			...
			for (const auto& current : vPTFX)
			{
				...
				{
				case SUB::PLAYEROPS:
				case SUB::SPOONER_SELECTEDENTITYOPS: __AddOption_entity(current, local_ped_id); break;
				case SUB::PTFXSUB: __AddOption_loopOnEntity(current, local_ped_id); break; // Subception
				case SUB::TRIGGERFXGUN: TriggerFxGun_catind::__AddOption_GunFx(current); break;
				}
			}
