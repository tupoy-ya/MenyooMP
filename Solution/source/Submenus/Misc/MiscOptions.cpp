...
namespace sub
{
	void MiscOps()
	{
		...
		bool misc_freecam_on = 0, misc_freecam_off = 0, autokill_plus = 0, autokill_minus = 0, misc_massacre_mode_on = 0, misc_massacre_mode_off = 0, jumpAround_on = 0, jumpAround_off = 0, blackout_off = 0, explosions_wp_plus = 0, explosions_wp_minus = 0,
		     //veh_collision_on = 0,
		    misc_pedPopu_off = 0, misc_vehPopu_off = 0;
		...
		enum eYscScriptTexterIndex
		{
			YSCSCRIPTTEXTER_LOAD,
			YSCSCRIPTTEXTER_UNLOAD
		};
		...
		if (yscScript_plus)
		{
			if (_____yscScript_texter_index < vYscScriptTexter.size() - 1)
				_____yscScript_texter_index++;
		}
		if (yscScript_minus)
		{
			if (_____yscScript_texter_index > 0)
				_____yscScript_texter_index--;
		}
		if (yscScript_input)
		{
			...
			{
				static const std::map<std::string, uint16_t> vYscStackSizes{{"achievement_controller", 1424},
				    {"ambient_sonar", 1424},
				    {"ambient_tonya", 1424},
		...
		if (autokill_plus)
		{
			if (loop_autoKillEnemies < 2)
				loop_autoKillEnemies++;
			return;
		}
		if (autokill_minus)
		{
			if (loop_autoKillEnemies > 0)
				loop_autoKillEnemies--;
			return;
		}

		if (misc_massacre_mode_on)
		{
			World::ClearWeatherOverride();
			World::SetWeatherOverTime(WeatherType::Thunder, 4000);
			return;
		}
		if (misc_massacre_mode_off)
		{
			World::ClearWeatherOverride();
			World::SetWeatherOverTime(WeatherType::ExtraSunny, 4000);
			return;
		}
		...
		if (explosions_wp_plus)
		{
			if (loop_explosion_wp < explosions_wp_names.size() - 1)
				loop_explosion_wp++;
			return;
		}
		if (explosions_wp_minus)
		{
	...
	void Timecycles_()
	{
		bool timecycles_reset = 0, timecycles_input = 0, heat_vision_on = 0, night_vision_on = 0, night_vision_off = 0, strength_plus = 0, strength_minus = 0;
		...
		for (auto& i : TimecycleModification::vTimecycles)
		{
			...
			AddOption(i.second, bModPressed);
			if (bModPressed)
			{
		...
		if (strength_plus)
		{
			if (menu_current_timecycle_strength < 3.0f)
		...
		if (strength_minus)
		{
			if (menu_current_timecycle_strength > 0.0f)
				menu_current_timecycle_strength -= 0.02f;
			TimecycleModification::SetModStrength(menu_current_timecycle_strength);
			return;
		}
	...
	void ClearAreaSub()
	{
		bool cleararea_vehicles = 0, cleararea_peds = 0, cleararea_objects = 0, cleararea_all = 0, cleararea_radius_plus = 0, cleararea_radius_minus = 0, cleararea_radius_input = 0;
		...
		if (*Menu::currentopATM == Menu::printingop)
			sub::Spooner::EntityManagement::DrawRadiusDisplayingMarker(GET_ENTITY_COORDS(local_ped_id, 1), _globalClearArea_radius);
		...
		if (cleararea_vehicles)
		{
			clear_area_of_entities(EntityType::VEHICLE, GET_ENTITY_COORDS(local_ped_id, 1), _globalClearArea_radius, {GET_VEHICLE_PED_IS_IN(local_ped_id, 0)});
			return;
		}
		if (cleararea_peds)
		{
			clear_area_of_entities(EntityType::PED, GET_ENTITY_COORDS(local_ped_id, 1), _globalClearArea_radius, {local_ped_id});
			return;
		}
		if (cleararea_objects)
		{
			clear_area_of_entities(EntityType::PROP, GET_ENTITY_COORDS(local_ped_id, 1), _globalClearArea_radius, {});
			return;
		}
		if (cleararea_all)
		{
			clear_area_of_entities(EntityType::ALL, GET_ENTITY_COORDS(local_ped_id, 1), _globalClearArea_radius, {local_ped_id, GET_VEHICLE_PED_IS_IN(local_ped_id, 0)});
			return;
		}
		...
		if (cleararea_radius_plus)
		{
			if (_globalClearArea_radius < FLT_MAX)
				_globalClearArea_radius += 0.5f;
			return;
		}
		if (cleararea_radius_minus)
		{
			if (_globalClearArea_radius > 0.0f)
				_globalClearArea_radius -= 0.5f;
			return;
		}
	}
	...
	void RadioSub_()
	{
		...
		bool mobile_radio_on = 0, mobile_radio_off = 0, radio_forward = 0;
		...
		if (ped.Equals(Game::PlayerPed())) // if this is our ped
		{
			...
			for (uint8_t i : vStationIds)
			{
				...
				AddTickol(GET_RADIO_STATION_NAME(i), GET_PLAYER_RADIO_STATION_INDEX() == i, stationPressed, null, frozenStation == i ? TICKOL::TICK2 : TICKOL::TICK, TICKOL::NONE, true);
				if (stationPressed)
				{
		...
		else // if this isn't our ped
		{
			...
			{
				bool stationPressed = false;
				AddOption(GET_RADIO_STATION_NAME(i), stationPressed, nullFunc, -1, false, true);
				if (stationPressed)
				{
					if (vehicle.Exists())
					{
	...
	namespace WaterHack_catind
	{
		...
		void Sub_WaterHack()
		{
			...
			bool radius_plus = 0, radius_minus = 0, height_plus = 0, height_minus = 0, radius_input = 0, height_input = 0;
	...
	namespace TVChannelStuff_catind
	{
		...
		std::map<std::string, std::string> vTvPlaylists{{"CNT", "PL_STD_CNT"}, {"Weazel News", "PL_STD_WZL"}, {"Weazel News Transition", "PL_MP_WEAZEL"}, {"Lotto Adverts", "PL_LO_CNT"}, {"Piswasser Adverts", "PL_LO_WZL"}, {"Righteous Slaughter", "PL_LO_RS"}, {"Righteous Slaughter Cutscene", "PL_LO_RS_CUTSCENE"}, {"Workout", "PL_SP_WORKOUT"}, {"Life Invader", "PL_SP_INV"}, {"Life Invader Explosion", "PL_SP_INV_EXP"}, {"Weazel Adverts", "PL_SP_PLSH1_INTRO"}, {"Fame or Shame Episode 1", "PL_LES1_FAME_OR_SHAME"}, {"Fame or Shame Episode 2", "PL_STD_WZL_FOS_EP2"}, {"Capolavoro", "PL_CINEMA_ARTHOUSE"}, {"The Loneliest Robot", "PL_CINEMA_CARTOON"}, {"Meltdown", "PL_CINEMA_ACTION"}, {"Cinema (No Meltdown)", "PL_CINEMA_MULTIPLAYER_NO_MELTDOWN"}, {"Jack Howitzer", "PL_WEB_HOWITZER"}, {"Kung Fu Rainbow LazerForce", "PL_WEB_KFLF"}, {"Republican Space Rangers", "PL_WEB_RANGERS"}, {"CCTV", "PL_MP_CCTV"}};
		...
		void Sub_TV()
		{
			...
			for (auto& pl : vTvPlaylists)
			{
				...
				AddOption(pl.first, bPlaylistPressed);
				if (bPlaylistPressed)
				{
	...
	namespace HudOptions_catind
	{
		...
		void Sub_HudOptions()
		{
			...
			AddToggle("Reveal Entire Minimap", loop_revealMinimap, bRevealMinimap_toggle, bRevealMinimap_toggle);
			if (bRevealMinimap_toggle)
			{
			...
			for (auto& h : std::vector<std::pair<int, std::string>>{{HudColour::PURE_WHITE, "Map Blips"}, {HudColour::WAYPOINT, "Waypoint"}, {HudColour::WHITE, "Pausemenu Text & Highlighting"}, {HudColour::PAUSE_BG, "PauseMenu Background"}, {HudColour::PAUSEMAP_TINT, "PauseMenu Map Tint"}, {HudColour::BLACK, "Selected Text And Notifications"}, {HudColour::FREEMODE, "Freemode"}, {HudColour::MICHAEL, "Michael"}, {HudColour::FRANKLIN, "Franklin"}, {HudColour::TREVOR, "Trevor"}})
			{
				bool bCompSelected = false;
				AddOption(h.second, bCompSelected, nullFunc, SUB::MSPAINTS_RGB);
				if (bCompSelected)
				{
					_hud_color_index      = h.first;
	...
	namespace GameCamOptions_catind
	{
		...
		void Sub_GameCamOptions()
		{
			...
			if (bShakeType_plus)
			{
				if (_shakeId < (int8_t)(vShakeNames.size() - 1))
				{
					_shakeId++;
					GameplayCamera::Shake(static_cast<CameraShake>(_shakeId), _shakeAmplitude);
				}
			}
			if (bShakeType_minus)
			{
			...
			if (bShakeAmp_plus)
			{
				_shakeAmplitude += 0.05f;
				if (!GameplayCamera::IsShaking())
				{
					GameplayCamera::Shake(static_cast<CameraShake>(_shakeId), _shakeAmplitude);
				}
				else
					GameplayCamera::ShakeAmplitude_set(_shakeAmplitude);
			}
			if (bShakeAmp_minus)
			{
				_shakeAmplitude -= 0.05f;
