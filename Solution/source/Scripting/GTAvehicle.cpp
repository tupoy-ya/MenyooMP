...
const std::vector<std::string> vValues_HornNames{"CMOD_HRN_TRK", "CMOD_HRN_COP", "CMOD_HRN_CLO", "CMOD_HRN_MUS1", "CMOD_HRN_MUS2", "CMOD_HRN_MUS3", "CMOD_HRN_MUS4", "CMOD_HRN_MUS5", "CMOD_HRN_SAD", "HORN_CLAS1", "HORN_CLAS2", "HORN_CLAS3", "HORN_CLAS4", "HORN_CLAS5", "HORN_CLAS6", "HORN_CLAS7", "HORN_CNOTE_C0", "HORN_CNOTE_D0", "HORN_CNOTE_E0", "HORN_CNOTE_F0", "HORN_CNOTE_G0", "HORN_CNOTE_A0", "HORN_CNOTE_B0", "HORN_CNOTE_C1", "HORN_HIPS1", "HORN_HIPS2", "HORN_HIPS3", "HORN_HIPS4", "HORN_INDI_1", "HORN_INDI_2", "HORN_INDI_3", "HORN_INDI_4", "HORN_LUXE1", "HORN_LUXE2", "HORN_LUXE3", "", "", "", "HORN_HWEEN1", "", "HORN_HWEEN2", "", "HORN_LOWRDER1", "", "HORN_LOWRDER2", "", "HORN_XM15_1", "", "HORN_XM15_2", "", "HORN_XM15_3", "", "CMOD_AIRHORN_01", "", "CMOD_AIRHORN_02", "", "CMOD_AIRHORN_03", ""};
...
std::string get_mod_slot_name(Vehicle vehicle, INT modType, bool gxt)
{
	...
	if (modType >= 0 && modType < vValues_ModSlotNames.size())
	{
		...
		case VehicleMod::SideSkirt:
			name = (vehModel.hash == VEHICLE_FAGGIO3) ? "TOP_ARCHCOVER" :
			    vehModel.IsBike()                     ? "CMM_MOD_S15" :
			                                            vValues_ModSlotNames[modType];
			break; // Air filter for bikes
		case VehicleMod::Roof:
			name = (vehModel.hash == VEHICLE_BAGGER) ? "CMOD_SB_T" : "CMOD_MOD_ROF";
			break; // Saddle bags for bagger
		case VehicleMod::AirFilter:
			if (vehModel.hash == VEHICLE_SULTANRS)
				name = "CMM_MOD_S15b";
			else
				name = vValues_ModSlotNames[modType];
		case VehicleMod::Struts:
			if (vehModel.hash == VEHICLE_SULTANRS || vehModel.hash == VEHICLE_BANSHEE2)
				name = "CMM_MOD_S16b";
			else
				name = vValues_ModSlotNames[modType];
		...
		case VehicleMod::Aerials:
			if (vehModel.hash == VEHICLE_SULTANRS)
				name = "CMM_MOD_S18b";
			else if (vehModel.hash == VEHICLE_BTYPE3)
				name = "CMM_MOD_S18c";
			else
				name = vValues_ModSlotNames[modType];
		case VehicleMod::Trim:
			if (vehModel.hash == VEHICLE_SULTANRS)
				...
				name = "CMM_MOD_S19c";
			else if (vehModel.hash == VEHICLE_VIRGO2)
				name = "CMM_MOD_S19d";
			else
				name = vValues_ModSlotNames[modType];
		case VehicleMod::Tank:
			if (vehModel.hash == VEHICLE_SLAMVAN3)
				name = "CMM_MOD_S27";
			else
				name = vValues_ModSlotNames[modType];
		case VehicleMod::Windows:
			if (vehModel.hash == VEHICLE_BTYPE3)
				name = "CMM_MOD_S21b";
			else
				name = vValues_ModSlotNames[modType];
		case VehicleMod::Unknown47:
			if (vehModel.hash == VEHICLE_SLAMVAN3)
				name = "SLVAN3_RDOOR";
			else
				name = vValues_ModSlotNames[modType];
...
std::string get_mod_text_label(Vehicle vehicle, INT modType, INT modValue, bool gxt)
{
	...
	switch (modType)
	{
	case VehicleMod::Engine: return "EMS Upgrade " + std::to_string(modValue + 1); break;
	case VehicleMod::Brakes:
	case VehicleMod::Transmission:
		return modValue >= brakenames.size() ? get_mod_slot_name(vehicle, modType, true) + std::to_string(modValue) : brakenames[modValue] + get_mod_slot_name(vehicle, modType, true);
		break;
	case VehicleMod::Horns:
		return modValue >= vValues_HornNames.size() ?
		    get_mod_slot_name(vehicle, modType, true) + std::to_string(modValue) :
		    (gxt && DOES_TEXT_LABEL_EXIST((PCHAR)vValues_HornNames[modValue].c_str()) ?
		            GET_FILENAME_FOR_AUDIO_CONVERSATION((PCHAR)vValues_HornNames[modValue].c_str()) :
		            vValues_HornNames[modValue]);
		break;
	case VehicleMod::Suspension:
		return modValue >= suspensionnames.size() ? get_mod_slot_name(vehicle, modType, true) + std::to_string(modValue) : suspensionnames[modValue] + get_mod_slot_name(vehicle, modType, true);
		break;
	case VehicleMod::Armor:
		return modValue >= armournames.size() ? "Armour Upgrade " + std::to_string((modValue + 1) * 20) + "%" :
		                                        (gxt && DOES_TEXT_LABEL_EXIST((PCHAR)armournames[modValue].c_str()) ?
		                                                GET_FILENAME_FOR_AUDIO_CONVERSATION((PCHAR)armournames[modValue].c_str()) :
		                                                armournames[modValue]);
		break;
	}

	const char* modName = GET_MOD_TEXT_LABEL(vehicle, modType, modValue);
	if (strlen(modName) > 3)
		return DOES_TEXT_LABEL_EXIST(modName) && gxt ? GET_FILENAME_FOR_AUDIO_CONVERSATION(modName) : modName;
...
void GTAvehicle::RoofState_set(VehicleRoofState value)
{
	...
	case VehicleRoofState::Opening: RAISE_CONVERTIBLE_ROOF(this->mHandle, value == VehicleRoofState::Opened); break;
	case VehicleRoofState::Closed:
	case VehicleRoofState::Closing: LOWER_CONVERTIBLE_ROOF(this->mHandle, value == VehicleRoofState::Closed); break;
	}
}
...
{
	SET_VEHICLE_ENGINE_ON(this->mHandle, value, true, false);
}
...
void GTAvehicle::OpenDoor(VehicleDoor door, bool loose, bool instantly, bool playSound)
{
	...
	if (playSound)
		PLAY_VEHICLE_DOOR_OPEN_SOUND(this->mHandle, 1);
}
void GTAvehicle::CloseDoor(VehicleDoor door, bool instantly, bool playSound)
{
	...
	if (playSound)
		PLAY_VEHICLE_DOOR_CLOSE_SOUND(this->mHandle, 1);
}
...
bool GTAvehicle::IsCargobobHookActive(CargobobHook hookType) const
{
	if (this->Model().IsCargobob())
	{
		switch (hookType)
		{
		case CargobobHook::Hook: return DOES_CARGOBOB_HAVE_PICK_UP_ROPE(this->mHandle) != 0;
		case CargobobHook::Magnet: return DOES_CARGOBOB_HAVE_PICKUP_MAGNET(this->mHandle) != 0;
		}
	}
	return false;
}
...
{
	if (!GET_VEHICLE_TYRES_CAN_BURST(this->mHandle))
		SET_VEHICLE_TYRES_CAN_BURST(this->mHandle, true);
...
void add_emblem_to_vehicle(GTAvehicle vehicle, GTAentity playerPed)
{
	...
	if (get_vehicle_info_for_emblem_pos(model, x, y, z, scale))
	{
		...
		ADD_VEHICLE_CREW_EMBLEM(vehicle.GetHandle(), playerPed.GetHandle(), vehicle.GetBoneIndex("chassis_dummy"), x.x, x.y, x.z, y.x, y.y, y.z, z.x, z.y, z.z, scale, 0, alpha);
	}
}
